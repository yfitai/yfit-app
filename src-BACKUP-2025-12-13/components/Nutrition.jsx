...
export default function Nutrition({ user: propUser }) {
 ...
 const loadData = async () => {
  try {
    // FORCE CLEAR: Remove this after one deployment cycle
    const today = new Date().toISOString().split('T')[0]
    const lastClear = localStorage.getItem('yfit_last_clear_date')
    
    if (lastClear !== today) {
      // Clear meals data every day
      localStorage.removeItem('yfit_demo_meals')
      localStorage.setItem('yfit_last_clear_date', today)
      console.log('Daily clear: Removed old meal data')
    }
