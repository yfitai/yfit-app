...
---
## Session: May 2, 2026 — Food Search Fix & Scanner Serving Size

### Food Search — All Foods Tab Broken (PRIORITY)
- [x] Remove Open Food Facts as English-language source (returns 0 results after filtering)
- [x] Rebuild searchFoods() with USDA-only for English (all 4 data types: Foundation, SR Legacy, Survey FNDDS, Branded)
- [x] Improve USDA relevance sorting (whole foods rank above obscure branded variants)
- [x] Update OFF Vercel proxy to accept `language` param for future multilingual support
- [x] Add `userLanguage` param to searchFoods() — when 'en', USDA only; when other language, USDA + OFF filtered to that language
- [x] Fix serving size handling for USDA-only results (consistent 10-option dropdown)
- [ ] Test: search "chicken", "banana", "greek yogurt", "almonds" — verify results appear immediately
- [ ] Test: My Foods tab still shows saved/custom foods correctly
- [ ] Test: Templates and My Foods still load from previous meals correctly
- [x] Deploy to Vercel and confirm live app search works (USDA returning results, OFF correctly disabled for English)

### Barcode Scanner — Serving Size Fix (SEPARATE ISSUE — HIGH RISK)
- [ ] Back up BarcodeScannerSelfContained.jsx as .jsx.backup before any changes
- [ ] Audit current scanner: identify where serving size label is read from barcode data
- [ ] Identify which field holds the correct per-serving nutrients vs per-100g nutrients
- [ ] Plan fix: use label serving size for nutrient calculation when available
- [ ] Implement fix with fallback to current working logic if serving size field is missing
- [ ] Test on at least 5 real barcodes before deploying
- [ ] Keep .jsx.backup as permanent rollback option

### Food Search — Additional Fixes (May 2, 2026)
- [x] Fix root cause: CSP in index.html was blocking yfit-deploy.vercel.app proxy calls
- [x] Fix searchUSDA() and searchOpenFoodFacts() to use CapacitorHttp (consistent native/web)
- [x] Add USDA ℹ️ tooltip: "Values may vary by brand. Scan barcode for exact label data."
- [x] Add accuracy note under Search/Scan/Template buttons on each meal card

### AI Plate Scan — "Snap & Estimate" Feature (FUTURE SESSION)
- [ ] Add camera button to meal cards for plate photo capture
- [ ] Upload photo to Supabase storage, send to LLM vision API
- [ ] LLM identifies foods and estimates portions (e.g. "grilled chicken ~150g, rice ~180g")
- [ ] Look up each identified food in USDA for nutrition values
- [ ] Show results with "AI Estimate ~±30%" badge — user adjusts quantities before logging
- [ ] Set clear user expectations: accurate for whole foods, rough for mixed dishes/restaurants

### Goals Page — Body Measurements Bug
- [x] Bug: Hips value resets to 34.3 inches after saving 44.0 inches
- [x] Also reflects incorrectly on Recomp page
- [x] Root cause: GoalsResults.jsx was a full duplicate Goals form that independently saved all measurements, defaulting empty fields to 0 and overwriting valid data
- [x] Fix: Replaced GoalsResults.jsx with a display-only component accepting metrics/firstName props
- [x] New GoalsResults shows: BMR, TDEE, target calories, BMI, body fat %, lean mass, macros (30/40/30 split), body type
- [ ] Verify fix: enter hips=44.0, save, reload page — should persist correctly
- [ ] Verify Recomp page shows correct hips value after fix
