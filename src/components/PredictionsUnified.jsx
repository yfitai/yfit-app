...
export default function PredictionsUnified({ user }) {
  ...
  const calculateTDEE = (wData = weightData, woData = workoutData, nData = nutritionData, userTDEE = null) => {
    ...
    try {
      ...
      console.log('[DEBUG TDEE] All woData:', woData.length, woData.map(w => ({ id: w.id, name: w.workout?.name || w.session_name, start: w.start_time })));
      console.log('[DEBUG TDEE] Weighted woData:', weightedWoData.length, weightedWoData.map(w => ({ id: w.id, name: w.workout?.name || w.session_name, start: w.start_time })));
