...
export default function MacroSettings({ user, leanBodyMassLbs, adjustedCalories, goalType, onMacrosUpdated }) {
  ...
  if (!macros) {
    return <div className="text-center py-4">{t('nutrition.calculatingMacros')}</div>
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-lg font-semibold text-gray-800 mb-4">🎯 {t('nutrition.macroTargets')}</h3>

      {/* Toggle */}
      <div className="flex items-center justify-between mb-4 p-4 bg-gray-50 rounded-lg">
        <div>
          <p className="font-medium text-gray-800">{t('nutrition.customMacroTargets')}</p>
          <p className="text-sm text-gray-600">
            {useCustomMacros
              ? t('nutrition.sliderAutoAdjust')
              : t('nutrition.usingLBMCalculation')}
          </p>
        </div>
        ...
        <div className="space-y-5 mb-4">
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">
              {slidersLocked
                ? `🔒 ${t('nutrition.tapEditToAdjust')}`
                : `✏️ ${t('nutrition.dragSlidersThenDone')}`}
            </p>
            {slidersLocked ? (
              <button
